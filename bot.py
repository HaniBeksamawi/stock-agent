...
def place_buy(symbol, price, state):
    try:
        ...
        if cash < POSITION_SIZE_USD:
            push_log(state, "Not enough cash for " + symbol, "warn")
            return False
        ...
        push_log(state, "BUY " + str(qty) + " x " + symbol + " @ $" + str(price), "trade")
        return True
    except Exception as e:
...
def scan_and_trade(state):
    ...
    if len(held) >= MAX_POSITIONS:
        push_log(state, "Max " + str(MAX_POSITIONS) + " positions held — monitoring only")
        return

    push_log(state, "🔍 Scanning " + str(len(watchlist)) + " stocks...")
    bought = 0
