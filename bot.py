...
POSITION_SIZE_PCT  = 0.07   # use 7% of total portfolio per trade (~$7,000)
...
def place_buy(symbol, price, state):
    try:
        ...
        if usable < price:
            ...
            return False
        # Use 7% of total portfolio value per trade (approx $7,000 on $100k)
        portfolio_value = float(get_account().get("portfolio_value", cash))
        trade_amount = min(portfolio_value * POSITION_SIZE_PCT, usable)
